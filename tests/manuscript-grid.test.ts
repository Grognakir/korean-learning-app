...
describe("layoutManuscript", () => {
  it("дополняет каждую строку до полной ширины пустыми клетками", () => {
    ...
    expect(rows[0][0]).toEqual({ char: "", caretIndex: 0, caretAfterIndex: 0 });
    expect(rows[0][1]).toEqual({ char: "가", caretIndex: 0, caretAfterIndex: 1 });
    for (let i = 2; i < 20; i++) {
      expect(rows[0][i]).toEqual({ char: "", caretIndex: 1, caretAfterIndex: 1 });
    }
  });
  ...
  it("клетка-отступ абзаца указывает на начало реального текста этого абзаца", () => {
    ...
    expect(rows[1][1]).toEqual({ char: "나", caretIndex: 2, caretAfterIndex: 3 });
  ...
  it("пара цифр занимает одну клетку и верно считает сырой индекс дальше", () => {
    ...
    expect(rows[0][0]).toEqual({ char: "", caretIndex: 0, caretAfterIndex: 0 });
    expect(rows[0][1]).toEqual({ char: "12", caretIndex: 0, caretAfterIndex: 2 });
    expect(rows[0][2]).toEqual({ char: "3", caretIndex: 2, caretAfterIndex: 3 });
  ...
  it("знак препинания, который открыл бы новую строку, прижимается к последней клетке предыдущей", () => {
    ...
    expect(rows[0]).toEqual([
      { char: "", caretIndex: 0, caretAfterIndex: 0 },
      { char: "나.", caretIndex: 0, caretAfterIndex: 2 },
    ]);
  });
});

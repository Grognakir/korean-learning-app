...
export function SessionProgress({ completed, total, label, showCompleted = false }: { completed: number; total: number; label: string; showCompleted?: boolean }) {
  const value = Math.min(total, Math.max(0, completed));
  return (
    <div className={styles.root}>
      <div className={styles.row}>
        <span>{label}</span>
        <span>{showCompleted ? value : Math.min(value + 1, total)} / {total}</span>
      </div>
